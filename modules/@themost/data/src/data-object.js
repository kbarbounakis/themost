...
function attrOf_(name, callback) {
    ...
    if (_.isNil(mapping)) {
        if (self.hasOwnProperty(name)) {
            ...
            return model.where(model.primaryKey).equal(self[model.primaryKey]).select(name).value((err, result) => {
    ...
    if (mapping.associationType==='association' && mapping.childModel === model.name) {
        ...
        if (self.hasOwnProperty(name)) {
            ...
            model.where(model.primaryKey).equal(this[model.primaryKey]).select(mapping.childField).flatten().value((err, result) => {
                if (err) { return callback(err); }
                self[name] = result;
                return callback(null, result);
            });
...
class DataObject {
    constructor(type, obj) {
        ...
        this.selector('new',
            ...
            function(callback) {
                ...
                const self = this;
                const model = self.$$model;
                model.inferState(self, (err, state) => {
                    if (err) { return callback(err); }
        ...
        if (_.isNil(mapping)) {
            ...
            return {
                value:function(callback) {
                    ...
                    function getValueWithCallback(callback) {
                        ...
                        if (self.hasOwnProperty(name)) {
                            ...
                            if (self.hasOwnProperty(model.primaryKey)) {
                                model.where(model.primaryKey).equal(self[model.primaryKey]).select(name).value((err, value) => {
                                    if (err) { return callback(err); }
                                    // set property
                                    self[name] = value;
                                    callback(null, value);
                                ...
                                model.inferState(self, (err, state) => {
                                    if (err) { return callback(err); }
                                    if (state===2) {
                                        model.where(model.primaryKey).equal(self.getId()).select(name).value((err, value) => {
                                            if (err) {
                                                return callback(err);
                                            }
                                            // set property
                    ...
                    return Q.promise((resolve, reject) => {
                        return getValueWithCallback((err, value) => {
                            if (err) {
                                return reject(err);
                            }
    ...
    attrOf(name, callback) {
        ...
        if (typeof callback !== 'function') {
            return Q.promise((resolve, reject) => {
                return attrOf_.call(self,  name, (err, result) => {
            ...
            if (field) {
                ...
                if (_.isNil(mapping)) {
                    if (self[model.primaryKey]) {
                        model.where(model.primaryKey).equal(self[model.primaryKey]).select(name).first((err, result) => {
                        ...
                        if (model.constraints.length===0) {
                            ...
                            if (arr.length===0) {
                                ...
                                q.select([name]).first((err, result) => {
                                    if (err) { callback(err); return; }
                                    let value = null;
                                    if (result) {
                                        value = result[name];
    ...
    save(context, callback) {
        ...
        if (typeof callback !== 'function') {
            return Q.promise((resolve, reject) => {
                return save_.call(self, context || self.context, err => {
                    if (err) { return reject(err); }
                    return resolve();
                });
            });
    ...
    remove(context, callback) {
        ...
        if (typeof callback !== 'function') {
            ...
            return Q.promise((resolve, reject) => {
                return remove_.call(self, context || self.context, err => {
                    if (err) { return reject(err); }
                    return resolve();
                });
            });
    ...
    getAdditionalModel() {
        ...
        return Q.promise((resolve, reject) => {
            try {
                ...
                self.attr(attrName, (err, additionalType) => {
                    try {
                        if (err) {
                            return reject(err);
                        }
    ...
    getAdditionalObject() {
        ...
        return Q.promise((resolve, reject) => {
            try {
                self.getAdditionalModel().then(additionalModel => {
                    try {
