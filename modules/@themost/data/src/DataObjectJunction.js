...
class DataObjectJunction {
    constructor(obj, association) {
        ...
        //get related model
        const relatedModel = this.parent.context.model(self.mapping.childModel);
        ...
        const baseAdapter = this.getBaseModel().viewAdapter;
        right[baseAdapter] = [QueryField.select(this.getValueField()).from(baseAdapter).$name];
        const field1 = QueryField.select(this.getObjectField()).from(baseAdapter).$name;
    ...
    getObjectField() {
        ...
        let attr = _.find(baseModel.attributes, x => {
        ...
        attr = _.find(baseModel.attributes, x => {
        ...
        if (typeof callback === 'undefined') {
            return Q.Promise((resolve, reject) => {
                return self.migrate(err => {
                    ...
                    // noinspection JSPotentiallyInvalidConstructorUsage
                    const superCount = DataObjectJunction.super_.prototype.count.bind(self);
    ...
    insert(obj, callback) {
        ...
        if (typeof callback === 'undefined') {
            return Q.Promise((resolve, reject) => {
                return insert_.call(self, obj, err => {
                    if (err) {
                        return reject(err);
                    }
                    return resolve(obj);
    ...
    remove(obj, callback) {
        ...
        if (typeof callback === 'undefined') {
            return Q.Promise((resolve, reject) => {
                return remove_.call(self, obj, err => {
                    if (err) {
                        return reject(err);
                    }
                    return resolve();
        ...
        if (err)
            ...
            async.eachSeries(arr, (item, cb) => {
                ...
                if (child.hasOwnProperty(self.mapping.childField)) {
                    insertSingleObject_.call(self, child, err => {
                        cb(err);
                    });
                }
                else {
                     ...
                     * @type {DataModel}
                     */
                    const relatedModel = self.parent.context.model(self.mapping.childModel);
                    //find object by querying child object
                    relatedModel.find(child).select(self.mapping.childField).first((err, result) => {
                        if (err) {
                            cb(err);
                        }
                        else {
                            ...
                            if (!result) {
                                ...
                                relatedModel.save(child, err => {
                                    if (err) {
                                        cb(err);
                                    }
                                    else {
                                        //insert relation between child and parent
                                        insertSingleObject_.call(self, child, err => { cb(err); });
                                    }
                                });
                            }
                            else {
...
function clear_(callback) {
    ...
    self.migrate(err => {
        ...
        // get parent id
        const parentValue = self.parent[self.mapping.parentField];
        // get relation model
        const baseModel = self.getBaseModel();
        // validate relation existence
        baseModel.where(self.getObjectField()).equal(parentValue).all((err, result) => {
            // if error occurred
            if (err) {
                return callback();
            }
...
function insertSingleObject_(obj, callback) {
    ...
    const parentValue = self.parent[self.mapping.parentField];
    const childValue = child[self.mapping.childField];
    //get relation model
    const baseModel = self.getBaseModel();
    //validate relation existence
    baseModel.silent(self.$silent).where(self.getObjectField()).equal(parentValue).and(self.getValueField()).equal(childValue).first((err, result) => {
        if (err) {
                ...
                relatedModel.find(child).select(self.mapping.childField).first((err, result) => {
                    if (err) {
                        ...
                        if (!result) {
                            ...
                            child[self.mapping.childField] = result[self.mapping.childField];
                            removeSingleObject_.call(self, child, err => {
                                cb(err);
                            });
                        }
                    }
