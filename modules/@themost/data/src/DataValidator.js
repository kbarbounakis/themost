...
class MaxLengthValidator extends DataValidator {
    ...
    validateSync(val) {
        ...
        let innerMessage = null;
        let message = sprintf.sprintf(this.message || MaxLengthValidator.DefaultMessage, this.maxLength);
        if (this.getContext() && (typeof this.getContext().translate === 'function')) {
            innerMessage = message;
...
class MinValueValidator extends DataValidator {
    ...
    validateSync(val) {
        ...
        if (val<this.minValue) {
            let innerMessage = null;
            let message = sprintf.sprintf(this.message || MinValueValidator.DefaultMessage, this.minValue);
            if (this.getContext() && (typeof this.getContext().translate === 'function')) {
                innerMessage = message;
...
class MaxValueValidator extends DataValidator {
    ...
    validateSync(val) {
        ...
        if (val>this.maxValue) {
            let innerMessage = null;
            let message = sprintf.sprintf(this.message || MaxValueValidator.DefaultMessage , this.maxValue);
            if (this.getContext() && (typeof this.getContext().translate === 'function')) {
                innerMessage = message;
...
class RangeValidator extends DataValidator {
    ...
    validateSync(val) {
        ...
        if (minValidator && maxValidator && (minValidation || maxValidation)) {
            let innerMessage = null;
            let message = sprintf.sprintf(this.message || RangeValidator.DefaultMessage, this.minValue, this.maxValue);
            if (this.getContext() && (typeof this.getContext().translate === 'function')) {
                innerMessage = message;
...
class DataValidatorListener {
    ...
    beforeSave(event, callback) {
        ...
        if (event.state === 1) {
            return event.model.validateForInsert(event.target).then(() => {
                return callback();
            }).catch(err => {
                return callback(err);
            });
            ...
            return event.model.validateForUpdate(event.target).then(() => {
                return callback();
            }).catch(err => {
                return callback(err);
            });
